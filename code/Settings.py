...
# Threading settings
processingThreads = 4				# Number of processing threads to work with
...
horizontalFlip = False				# True if the camera left/right needs to be flipped
...
firstStraightMin = 1.0				# Minimum number of seconds straight forward
firstStraightMax = 2.0				# Maximum number of seconds straight forward

#######################################################################################
# These settings define the layout of the track and are used for working out how far  #
# the MonsterBorg has travelled, they only affect the Race Code Functions             #
#######################################################################################

# MonsterBorg movement details
monsterSpeed = 1.44					# Speed the MonsterBorg moves at with 100% power in m/s
monsterSpeedFullSteering = 1.2   	# Speed the MonsterBorg moves at with 100% steering in m/s
...
#maxPower = 0.50					# Maximum of allowed drive output - Bench testing
...
blackMaxR = 90						# Maximum red level for black detection (0-255)
blackMaxG = 100						# Maximum green level for black detection (0-255)
blackMaxB = 50						# Maximum blue level for black detection (0-255)

# Image start line identification tuning
startCrossedSeconds = 0.1			# Number of seconds to wait after the start line is seen before announcing crossing
...
startMaxG = 60						# Maximum green level in the start detection zone
startMaxB = 60						# Maximum blue level in the start detection zone
...
blueGain  = 1.45					# Gain to make the blue channel equal
...
# PID control values
Kp0 = 0.10							# P term for d0 input (Offset)
Ki0 = 0.00							# I term for d0 input (Offset)
Kd0 = 0.10							# D term for d0 input (Offset)
Kp1 = 0.055							# P term for d1 input (Angle)
Ki1 = 0.00							# I term for d1 input (Angle)
Kd1 = 0.055							# D term for d1 input (Angle)
...
# Original Value - lightsChangeThreshold: 50.0 
lightsChangeThreshold = 25.0		# Minimum colour difference to flag as a change in 
##################################################
light colour 
...
monsterDistancePerFrame = monsterSpeed / float(frameRate)
trackChangePerLane = (trackLengthOuter - trackLengthInner) / float(lanes)
stuckDetectedDistanceCorrection = (stuckIdenticalFrames + stuckOverrideFrames) * (monsterSpeed / 4.0)
